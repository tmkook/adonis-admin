...
// Token management
router.get('auth/tokens', [SystemController, 'accessTokens']).as('admin_tokens')
router.post('auth/tokens/refresh', [SystemController, 'refreshToken']).as('admin_token_refresh')
router.delete('auth/tokens/:id', [SystemController, 'outAccessToken']).as('admin_token_delete')
...
```bash
node ace admin:make-resource ModelName
```
...
1. 用户调用 `showTotpSecret` 获取密钥
2. 使用 Google Authenticator 扫描返回的 QR Code
3. 首次输入账号密码后，返回 totp 开启状态和密钥
4. 再次请求登录提交验证码和密钥
...
sequenceDiagram
    ...
    User->>API: POST /auth/login (username, password, code, token)
    API->>API: 验证TOTP验证码
    API->>DB: 创建访问令牌
    DB-->>API: 令牌信息
